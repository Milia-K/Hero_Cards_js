...
const data = [
    ...
    "superpowers": "непробиваемость, суперсила, полёт, самоисцеление, суперзрение и суперслух",
    ...
    "friends": "Мстители",
    "superpowers": "высокий уровень интеллекта, широкие познания науки и техники, бронекостюмы",
    ...
    "superpowers": "сверхчеловеческие рефлексы, «паучье чутьё», производство паутины",
    ...
    "friends": "Мстители",
    "superpowers": "всё, что может бог, плюс молот Мьелнир, телепортация в пространстве и времени",
...
class Heroes {
    constructor(name, universe, alterego, superpowers, url) {
        this.name = name;
    ...
    getInfo(){
        return `${this.name} <br />
        Вселенная: ${this.universe} <br />
        Альтер эго: ${this.alterego} <br />
        Суперсилы: ${this.superpowers} <br />
        <img src =${this.url}> <br />

        <div>Рейтинг</div>
        <span><button class='btn_delete'>&#10008;</button></span>
        <span><input class='input' type="number" id="quantity" min="1" max="10" placeholder='&#9734;'></span>
        <button class='btn'>&#10004;</button>`
    }
}

data.slice(0,9).forEach((item) => {
    const hero = new Heroes (item.name, item.universe, item.alterego, item.superpowers, item.url);
    const div = document.createElement('div');
    div.classList.add('card');
    div.innerHTML = hero.getInfo();
    ...
    const btn_delete = div.querySelector('.btn_delete')
    ...
    function saveData() {
        ...
        if (rating) {
            localStorage.setItem(hero.name, rating);
        }
    }
    const input = div.querySelector('.input')
